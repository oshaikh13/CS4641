...
class PolicyIterationSolver(BaseSolver):
    def __init__(self, env, discount_factor=0.9, same_last_step_threshold=10, max_policy_eval_steps=None, verbose=False):
        self._env = env.unwrapped

        self._policy = np.ones([self._env.nS, self._env.nA]) / self._env.nA
        self._discount_factor = discount_factor
        self._steps = 0
        self._same_last_step = 0
        self._last_delta = 0
        self._step_times = []
        self._policy_stable = False
        self._max_policy_eval_steps = max_policy_eval_steps
        self._same_last_step_threshold = same_last_step_threshold
    ...
    def step(self):
        ...
        if delta == self._last_delta: self._same_last_step += 1
        else: self._same_last_step == 0

        if self._same_last_step > self._same_last_step_threshold:
            self._policy_stable = True
